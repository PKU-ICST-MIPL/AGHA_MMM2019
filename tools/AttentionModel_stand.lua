...
function Attention.baseModel(feat,feat_size,att_seq,seq_size,att_size, att_hid_size)
    local att = nn.View(-1, seq_size)(att_seq)         -- (batch * att_size) * seq_size
    local att_h, dot
    print('Attention.baseModel: base attention model...')
    print('feat_size: '..feat_size..' seq_size: '..seq_size..' att_size: '..att_size..' att_hid_size: ' .. att_hid_size)
    ...
    local att_res = nn.MixtureTable(2){weight, att_seq}   -- batch * feat_size <- (batch * att_size, batch * att_size * seq_size)
    return att_res
end
function Attention.baseModel_v2(feat,feat_size,att_seq,seq_size,att_size, att_hid_size)
    local att = nn.View(-1, seq_size)(att_seq)         -- (batch * att_size) * seq_size
    local att_h, dot
    print('Attention.baseModel_v2: base attention model...')
    print('feat_size: '..feat_size..' seq_size: '..seq_size..' att_size: '..att_size..' att_hid_size: ' .. att_hid_size)
    if att_hid_size > 0 then
        att = nn.Linear(seq_size, att_hid_size)(att)        -- (batch * att_size) * att_hid_size
        att = nn.View(-1, att_size, att_hid_size)(att)      -- batch * att_size * att_hid_size
    ...
    local weight = nn.SoftMax()(dot)
    local att2 = nn.View(-1, att_size, seq_size)(att_seq)
    local att_res = nn.MixtureTable(2){weight, att2}   -- batch * feat_size <- (batch * att_size, batch * att_size * seq_size)
    return att_res
end
