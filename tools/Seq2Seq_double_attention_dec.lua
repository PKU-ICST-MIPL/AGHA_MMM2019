-------------------------------------------------------------------------------------
------[dual memory recurrent model]--
-------------------------------------------------------------------------------------
require 'nn'
require 'torch'
local LSTM_tools = require 'tools.LSTM_double_state_my'
...
function layer:__init(opt)
  ...
  self.local_feat_size = opt.att_feat_size
  ...
  -- build up models
  print('build double state model with attention....')
  print(opt)
  ...
  --opt_enc.att_type = 2   -- set attention type 1:local_prev_h ,2:prev_h, 3:content , default is 1  
  ...
  opt_dec.att_feat_size = opt.featsize -- attention 1 : global feature
  opt_dec.att_length = opt.feat_seq_length

  opt_dec.att2_feat_size = opt.att_feat_size -- attention 2: local feature (object, relation, region)
  opt_dec.att2_length = opt.feat_seq_length

  print('Seq2Seq __init: to build lstm_soft_att_double_state_res ...')
  self.encoder = LSTM_tools.lstm_soft_att_double_state_res(opt_enc) -- feature encoder
  if opt.dec_type == 'weight_scalar' then
    print('Seq2Seq __init: to build lstm_language_att_double with scalar ...')
    self.decoder = LSTM_tools.lstm_language_att_double(opt_dec) -- sentence decoder
  elseif opt.dec_type == 'weight_learn' then
    print('Seq2Seq __init: to build lstm_language_att_double_weight ...')
    self.decoder = LSTM_tools.lstm_language_att_double_weight(opt_dec) -- sentence decoder
  end
...
function layer:updateOutput(input)   -- input = {global feature, word feature, local feature}
  --print('Seq2Seq:updateOutput: called by forward() ...')
  ...
  local local_feats = input[3] -- sequence length * batch size * localnum * feature size
  --print(string.format('context size: %dx%dx%d', context:size(1), context:size(2), context:size(3)))
  --print(string.format('words size: %dx%d', words:size(1), words:size(2)))
  --print('local_feats size: \n', local_feats:size())
  ...
  --self.fc_scores = torch.CudaTensor(self.sentence_length+1, batch_size, self.vocabsize+1):zero()
  --print('self.hiddensize = ', self.hiddensize)
  ...
  local enc_att_res = torch.CudaTensor(self.feat_seq_length, batch_size, self.local_feat_size):zero()
  local dec_ct={}
  local dec_ht={}
  ...
  for t=1, self.feat_seq_length do
      ...
      enc_ct[t], enc_ht[t],local_enc_ct[t], local_enc_ht[t], enc_att_res[t] = unpack(enc_out)
  end
  ...
  local att2_seq = enc_att_res:clone():transpose(1,2):contiguous()
  ...
  for t=1, self.sentence_length+1 do
      ...
      --print('t= ', t, 'embed_out size :\n', embed_out:size())
      self.dec_inputs[t] = {embed_out, att_seq, att2_seq, dec_ct[t-1], dec_ht[t-1] }
...
function layer:updateGradInput(input, gradOutput)  
  ...
  --print('Seq2Seq:updateGradInput: called by backward() ...')
  --print(input[1]:size())
  --print(input[2]:size())
  --print(input[3]:size())
  ...
  local datt2_seq = torch.CudaTensor(batch_size, self.feat_seq_length, self.local_feat_size):zero()
  ...
  for t=self.dec_max,1,-1 do
      ...
      local dembed, datt2_seq_temp
      dembed,_,datt2_seq_temp, ddec_ct[t-1], ddec_ht[t-1] = unpack(self.decoder_clones[t]:backward(self.dec_inputs[t], ddec_out))
      --datt2_seq:add(datt2_seq_temp)
      local it = self.embed_inputs[t]
      self.embed_clones[t]:backward(it, dembed)
  end
  
  denc_att_res = datt2_seq:clone():transpose(1,2):contiguous()
  -- for encoder and decoder connection
  ...
  for t=self.feat_seq_length,1,-1 do
    local denc_out = {denc_ct[t], denc_ht[t],local_denc_ct[t], local_denc_ht[t], denc_att_res[t]}
...
function layer:sample(input, opt)
  ...
  local batch_size = context:size(2)
  local att_seq = context:clone():transpose(1,2):contiguous()
  --self.alpha  = torch.CudaTensor(self.feat_seq_length, batch_size, 49):zero()
  ...
  for t=1, self.feat_seq_length do
    ...
    enc_ct, enc_ht,local_enc_ct, local_enc_ht, enc_att_res[t]= unpack(enc_out)
  end
  --print(string.format('enc_ct: %dx%d', enc_ct:size(1), enc_ct:size(2)))
  --print(string.format('local_enc_ht: %dx%d', local_enc_ht:size(1), local_enc_ht:size(2)))
  ...
  --print(string.format('dec_ct: %dx%d', dec_ct:size(1), dec_ct:size(2)))
  --print(string.format('dec_ht: %dx%d', dec_ht:size(1), dec_ht:size(2)))
  local att2_seq = enc_att_res:clone():transpose(1,2):contiguous()
  if opt.beam_size ~= nil then return self:beam_search({{dec_ct,dec_ht},{att_seq,att2_seq}}, {beam_size = opt.beam_size}) end
...
function layer:beam_search(input, opt)
  ...
  local att_seq = input[2][1] -- for temporal attention
  local att2_seq = input[2][2] -- for temporal attention2
  
  local feat_dim = att_seq:size(3)
  local feat2_dim = att2_seq:size(3)
  ...
  for k=1, batch_size do
    ...
    local state_c = input[1][1]:narrow(1,k,1):expand(beam_size, self.hiddensize_dec)
    local state_h = input[1][2]:narrow(1,k,1):expand(beam_size, self.hiddensize_dec)
    ...
    local att2_seq_temp = att2_seq:narrow(1,k,1):expand(beam_size, self.feat_seq_length, feat2_dim):contiguous()
    ...
    for t=1,self.sentence_length+1 do
      ...
      local dec_inputs = {embed_out, att_seq_temp, att2_seq_temp, state_c, state_h}
...
function crit:updateOutput(input, seq)
  ...
  --print(string.format('L N Mp1 D = %d %d %d %d ', L, N, Mp1, D))
