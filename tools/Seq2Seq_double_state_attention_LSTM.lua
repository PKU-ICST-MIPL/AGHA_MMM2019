...
function layer:__init(opt)
  ...
  print('Seq2Seq __init: to build lstm ...')
  self.encoder = LSTM_tools.lstm(opt_enc) -- feature encoder
  print('Seq2Seq __init: to build lstm_language_att ...')
  self.decoder = LSTM_tools.lstm_language_att(opt_dec) -- sentence decoder
...
function layer:updateOutput(input)   -- input = {global feature, word feature, local feature}
  ...
  for t=1, self.feat_seq_length do
      ...
      self.enc_inputs[t] = {context[t],enc_ct[t-1], enc_ht[t-1]}
      ...
      enc_ct[t], enc_ht[t] = unpack(enc_out)
  end
  ...
  dec_ct[0] = enc_ct[self.feat_seq_length]:clone()
  dec_ht[0] = enc_ht[self.feat_seq_length]:clone()
  --print('dec_ct[0] size \n', dec_ct[0]:size())
  --print('dec_ht[0] size \n', dec_ht[0]:size())
...
function layer:updateGradInput(input, gradOutput)  
  ...
  denc_ct[self.feat_seq_length] = ddec_ct[0]:clone()
  denc_ht[self.feat_seq_length] = ddec_ht[0]:clone()
  ...
  for t=self.feat_seq_length,1,-1 do
    ...
    local denc_out = {denc_ct[t], denc_ht[t]}
    _,denc_ct[t-1], denc_ht[t-1] = unpack(self.encoder_clones[t]:backward(self.enc_inputs[t], denc_out))
  end
...
function layer:sample(input, opt)
  ...
  for t=1, self.feat_seq_length do
    ...
    local enc_out = self.encoder:forward{context[t],enc_ct,enc_ht}
    enc_ct, enc_ht = unpack(enc_out)
  end
