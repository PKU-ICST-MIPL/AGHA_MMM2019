...
local LSTM_tools = {}
print('LSTM_double_state: file version LSTM tools')
local dropout = 0.8
-------------------------------------------------------------------------
...
function LSTM_tools.lstm_soft_att_double_state(opt)
    ...
    elseif att_type == 3 then
        ...
        att_res = Attention.baseModel(x,input_size,att_seq, feat_size, att_size,att_hid_size) --batch_size * feat_size
        print(att_res)
    end
...
function LSTM_tools.lstm_soft_att_double_state_res(opt)
    -- Model parameters 
    -- for input feat and lstm
    local rnn_size = opt.rnn_size
    local input_size = opt.input_size
    ...
    local att_hid_size = opt.att_hid_size      -- for attention hidden size
    local att_type = opt.att_type or 3
    print('.......................LSTM Soft Attention with Double State Parameters res ..............')
    print(opt)
    local inputs = {}
    local outputs = {}
    for i=1,6 do
...
function LSTM_tools.lstm_language_att(opt)
    ...
    --return nn.gModule({x, att_seq, prev_c, prev_h}, {next_c, next_h, proj})
end

function LSTM_tools.lstm_language_att_double(opt)
    -- Model parameters
    local rnn_size = opt.rnn_size
    local input_size = opt.input_size
    -- for attention seq
    ...
    local feat2_size = opt.att2_feat_size
    local att2_size = opt.att2_length

    print('.......................LSTM Language Model with attention parameters double..............')
    print(opt)
    local x = nn.Identity()()         -- batch * input_size -- embedded caption at a specific step
    local att_seq = nn.Identity()()   -- batch * att_size * feat_size -- the image patches
    local att2_seq = nn.Identity()()
    local prev_c = nn.Identity()()
    local prev_h = nn.Identity()()

    ------------ Attention part --------------------
    local att_res = Attention.baseModel(prev_h,rnn_size,att_seq,feat_size, att_size, att_hid_size)
    local att2_res = Attention.baseModel(prev_h,rnn_size,att2_seq,feat2_size, att2_size, att_hid_size)

    ------------ End of attention part -----------

    -- weight att2_res
    att2_res = nn.MulConstant(0.1)(att2_res)

    ------------- LSTM main part --------------------
    function buildGate(in_size,f_size,f2_size, xf, prev_hf, att_resf, att2_resf)
        local i2h = nn.Linear(in_size, rnn_size)(xf)
        local h2h = nn.Linear(rnn_size, rnn_size, false)(prev_hf)
        local att_add = nn.Linear(f_size, rnn_size, false)(att_resf)
        local att2_add = nn.Linear(f2_size, rnn_size, false)(att2_resf)
        return nn.CAddTable()({i2h, h2h,att_add,att2_add})
    end

    local in_gate = nn.Sigmoid()(buildGate(input_size,feat_size,feat2_size, x, prev_h, att_res, att2_res))
    local forget_gate = nn.Sigmoid()(buildGate(input_size,feat_size,feat2_size, x, prev_h, att_res, att2_res))
    local out_gate = nn.Sigmoid()(buildGate(input_size,feat_size,feat2_size, x, prev_h, att_res, att2_res))
    local in_transform = nn.Tanh()(buildGate(input_size,feat_size,feat2_size, x, prev_h, att_res, att2_res))
    ...
    return nn.gModule({x, att_seq, att2_seq, prev_c, prev_h}, {next_c, next_h, logsoft})
    --return nn.gModule({x, att_seq, prev_c, prev_h}, {next_c, next_h, proj})
end

function LSTM_tools.lstm_language_att_double_weight(opt)
    -- Model parameters
    local rnn_size = opt.rnn_size
    local input_size = opt.input_size
    -- for attention seq
...
function LSTM_tools.lstm_language_att_v2(opt)
    -- Model parameters
    local rnn_size = opt.rnn_size
    local input_size = opt.input_size
    -- for attention seq
    ...
    local att_size = opt.att_length
    local output_size = opt.output_size     -- for word prediction
    print('.......................LSTM Language Model with attention parameters v2..............')
    print(opt)
    local x = nn.Identity()()         -- batch * input_size -- embedded caption at a specific step
    local att_seq = nn.Identity()()   -- batch * att_size * feat_size -- the image patches
    local prev_c = nn.Identity()()
    ...
    ------------ Attention part --------------------
    local att_res = Attention.baseModel_v2(prev_h,rnn_size,att_seq,feat_size, att_size, att_hid_size)
