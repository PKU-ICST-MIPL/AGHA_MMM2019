...
function layer:__init(opt)
  ...
  -- build up models
  print('build double state model with attention v2 ...')
  print(opt)
  ...
  print('Seq2Seq __init: to build lstm_soft_att_double_state ...')
  self.encoder = LSTM_tools.lstm_soft_att_double_state(opt_enc) -- feature encoder
  print('Seq2Seq __init: to build lstm_language_att ...')
  self.decoder = LSTM_tools.lstm_language_att_v2(opt_dec) -- sentence decoder
  ...
  self.connectTable_att = nn.JoinTable(2)
  print('done....')
end
...
function layer:updateOutput(input)   -- input = {global feature, word feature, local feature}
  ...
  --for v2, don't use global feature as attention
  --local att_seq = context:clone():transpose(1,2):contiguous()
  --print('att_seq size: \n', att_seq:size())
  --print(torch.type(context:clone():transpose(1,2):contiguous()))
  ...
  self.connect_input_hs = {}
  self.connect_input_att = {}
  for t=1, self.feat_seq_length do
    self.connect_input_hs[t] = {enc_ht[t], local_enc_ht[t]}
    self.connect_input_att[t] = self.connectTable_h:forward(self.connect_input_hs[t])
  end
  self.dec_att_input = self.connectTable_att:forward(self.connect_input_att)
  local att_seq = self.dec_att_input
  --print(att_seq:size())
  ...
  for t=1, self.sentence_length+1 do
      ...
      self.dec_inputs[t] = {embed_out, att_seq, dec_ct[t-1], dec_ht[t-1] }
...
function layer:updateGradInput(input, gradOutput)  
  ...
  for t=self.dec_max,1,-1 do
      ...
      local dembed
      dembed,_,ddec_ct[t-1], ddec_ht[t-1] = unpack(self.decoder_clones[t]:backward(self.dec_inputs[t], ddec_out))
...
function layer:sample(input, opt)
  ...
  local dec_ht
  for t=1, self.feat_seq_length do
    local enc_out = self.encoder:forward{context[t],local_feats[t],enc_ct[t-1],enc_ht[t-1],local_enc_ct[t-1], local_enc_ht[t-1]}
    enc_ct[t], enc_ht[t],local_enc_ct[t], local_enc_ht[t]= unpack(enc_out)
  end
  ...
  dec_ct = self.connectTable_c:forward{enc_ct[self.feat_seq_length], local_enc_ct[self.feat_seq_length]}
  dec_ht = self.connectTable_h:forward{enc_ht[self.feat_seq_length], local_enc_ht[self.feat_seq_length]}
  local connect_att_inputs = {}
  for t=1, self.feat_seq_length do
    connect_att_inputs[t] = self.connectTable_h:forward{enc_ht[t], local_enc_ht[t]}
  end
  local att_seq = self.connectTable_att:forward(connect_att_inputs)
  assert(att_seq:size(2)==self.feat_seq_length*self.hiddensize_dec)

  --print(string.format('dec_ct: %dx%d', dec_ct:size(1), dec_ct:size(2)))
  --print(string.format('dec_ht: %dx%d', dec_ht:size(1), dec_ht:size(2)))
  
  if opt.beam_size ~= nil then return self:beam_search({{dec_ct,dec_ht},att_seq}, {beam_size = opt.beam_size}) end
  ...
  for t=1,self.sentence_length+1 do
      ...
      local dec_input = {embed_out,att_seq,dec_ct, dec_ht}
...
function layer:beam_search(input, opt)
  ...
  local att_feat_dim = att_seq:size(2)
  ...
  for k=1, batch_size do
    ...
    local done_beams = {}
    local att_seq_temp = att_seq:narrow(1,k,1):expand(beam_size, att_feat_dim):contiguous() 
    for t=1,self.sentence_length+1 do
      local embed_out, it, sampleLogprobs
      ...
      local dec_inputs = {embed_out, att_seq_temp, state_c, state_h}
